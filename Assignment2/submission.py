...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    for epoch in range(numEpochs):
        ...
        for x, y in trainExamples:
            ...
            # for key in weights:
            #     weights[key] = weights[key] - alpha * (h-y)*feature_extractor[key]
        train_err = evaluatePredictor(trainExamples, function)
        print(f'Training Error: ({epoch} epoch): {train_err}')
        test_err = evaluatePredictor(validationExamples, function)
        print(f'Validation Error: ({epoch} epoch): {test_err}')
    # END_YOUR_CODE
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        """
        # BEGIN_YOUR_CODE (our solution is 4 lines of code, but don't worry if you deviate from this)
        length = random.randint(1, len(weights))
        random_keys = random.sample(weights.keys(), length)
        phi = {k: weights[k] for k in random_keys}
